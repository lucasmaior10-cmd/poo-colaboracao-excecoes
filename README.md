...
3. Execute `make run` para observar `Painel: 0` e `Sem calibracao: leitura indevida: 20`; depois use `make test ETAPA=A` para ler a primeira pendencia.
4. Complete a consulta do painel em `include/estacao.hpp` e `src/estacao.py`; teste e registre um commit.
5. Complete a regra de calibracao nos mesmos arquivos; confira que `make run` agora mostra `Sem calibracao: sem leitura` e que `make test ETAPA=A` passa. Registre outro commit.
...
O GitHub Pages usa `main` e `/docs`. Use `make run` para a conferencia visual local. O teste inicial deve falhar na consulta do painel; apos o incremento guiado deve falhar na calibracao. Valide a solucao de referencia em uma copia privada antes de publicar alteracoes no starter. A CI executa testes funcionais nos pushes da branch prevista e na PR; a execucao da `main` confirma a baseline executavel.
